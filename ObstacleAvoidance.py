...
def execute_ObsAv(positions,head,obstacles):
    ...
    while exit_node==False:
        ...
        while (node):
            ...
            if node.nxt:
                plt.plot([node.pos[0], node.nxt.pos[0]], [node.pos[1], node.nxt.pos[1]], color='red')

            forceVector = node.pos + node.calcForces(obstacles) * 100
            plt.plot([node.pos[0], forceVector[0]], [node.pos[1], forceVector[1]], color='green', linewidth=1)
            plt.plot(node.pos[0], node.pos[1], marker='o', markersize=3, color='blue')
            node.applyForces(obstacles)
            add_points = node.smoothCurve(15,node) #* Smaller values increase de smoothnes 5-> para 2D
        ...
        if all(flag)==True:
            ...
            for h, point in enumerate(path):
                plt.text(point[0], point[1], str(h), fontsize=8, color='black')
...
class Node:
    def __init__(self, pos = None, prv = None, nxt = None,
                locked = False, mass = 0.5, jointRigidity = 1.0):
    ...
    def calcForces(self,obstacles):
        ...
        if not self.locked:
            ...
            #totForce = totForce + self.getDVForce(500)
...
#       return force
    ...
    def smoothCurve(self, fPedMax, node):
        ...
        if self.getCurvature() >= fPedMax:
            """ print("Next node:",np.linalg.norm(self.pos - self.nxt.pos) > self.minDist)
            print("Segundo next node: ",np.linalg.norm(self.pos - self.prv.pos) > self.minDist) """
            """ print("Pos actual: ",self.pos)
            print("Pos siguiente: ",self.nxt.pos)
            print("Valor dentro:",np.linalg.norm(self.pos - self.nxt.pos)) """
            #time.sleep(1000)
